...
func (s *State) Transit() {
	var instruction = selector.Mux(api, s.pc, s.instructions...)
	var op, memRead, memWrite, jmp, memAddr, instructionOp = decodeInstruction(instruction)

	s.cache.SelectAddr(memAddr)
	var read = s.cache.Read(memAddr)

	var operand = selector.Mux(api, memRead, instructionOp, read)
	...
	var res = selector.Mux(api, op, add, sub, mul, api.Sub(0, sub))
	s.acc = selector.Mux(api, jmp, res, s.acc)

	var write = selector.Mux(api, memWrite, read, res)
	s.cache.Write(write)
...
func init() {
	solver.RegisterHint(decodeHint)
}
